...
public class VariableTests  extends WithApplicationAndIsoletedXfl {
    ...
    @Test
    public void getFuzzyVariableForSystem() {
        try{
    ...
    @Test
    public void getFuzzyVariableForRulebase() {

        try{
            xfuzzy.lang.Rulebase fRB = this.testSystem.getSpecification().getRulebases()[0];


            xfuzzy.lang.Variable fvar_input1 = Variable.getFuzzy(this.testSystem,fRB,0,Variable.INPUT);
            assertEquals("t1 vi1", fvar_input1.toXfl());

            xfuzzy.lang.Variable fvar_output2 = Variable.getFuzzy(this.testSystem,fRB,1,Variable.OUTPUT);
            assertEquals("t1 vo2", fvar_output2.toXfl());
        }
        catch(Exception e){
            fail(e.getMessage());
        }
